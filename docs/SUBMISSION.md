...
Judges can explore a separate public Cloud Run service at `https://atlas-public-demo-jguwjegiqq-uc.a.run.app`. It is visibly labelled and read-only. All 64 controls have visible, model-labelled rulings: 62 seeded deterministic decisions plus two sanitised rulings captured from the verified private Gemini run. Its runtime makes no model calls, has no direct project IAM role bindings and exposes no mutation, upload, SSE, briefing, docs or internal-sweep route.
